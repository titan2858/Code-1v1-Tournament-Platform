...
const Round = () => {
    ...
    if (!user) {
      ...
      const fetchData = async () => {
        try {
          ...
          const ID=user.id;
          const res = await axios.get(`${process.env.REACT_APP_API_URL}/api/tournament/match/getProblemID`,  { params: { ID }});
  ...
  useEffect(() => {
    const fetchTime = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}/api/tournament/getTime`, { params: { roomId } });
        ...
        if (timeLeftInMilliseconds <= 0) {
          if (!resultCalculated) {
            try {
              await axios.post(`${process.env.REACT_APP_API_URL}/api/tournament/match/calculateResult`, { roomId });
